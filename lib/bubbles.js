export const nestScopesWithLines = (scopes, lines) => {
  ...
  function nestScopesWithLinesRecursively(scope, lines, level = 0) {
    ...
    const additionalScopeInformation = scopes.find((s) =>
      matchScopeBlocks(s, scope),
    );
    ...
    const nestedScopesOrLines = [
      ...new Set(
        linesInRange.map((line) => {
          const childScopeWhichOwnsLine = childrenScopesWithLines.find(
            (childScope) => childScope.lines.includes(line),
          );
    ...
    const childrenMaxLevel = Math.max(
      ...childrenScopesWithLines.map(({ levels }) => levels),
    );
